...
async function sendUserPhotos(admin: any, senderId: string): Promise<string> {
  const { data: rows } = await admin
    ...
    .eq("facebook_user_id", senderId).order("created_at", { ascending: false });
  const list = (rows ?? []) as any[];
  if (!list.length) return JSON.stringify({ ok: true, count: 0, message: "لا توجد لديك أي صور محفوظة بعد." });
  let sent = 0;
  for (const row of list) {
    const { data: signed, error } = await admin.storage.from("user-photos").createSignedUrl(row.storage_path, 3600);
    if (error || !signed?.signedUrl) { console.error("[photos] sign failed", error); continue; }
  ...
  return JSON.stringify({
    ok: true, count: sent,
    instruction: "الصور تم إرسالها بالفعل كمرفقات مباشرة عبر Messenger. لا تكتب أي روابط، ولا Markdown، ولا ![](...) ، ولا عناوين http/https في ردك. اكتفِ برسالة قصيرة جداً بالعربية مثل: «تفضّل، هذه صورك المحفوظة 📷» ثم اعرض ما يمكن للمستخدم فعله لاحقاً (حفظ المزيد / الحذف).",
  });
}
...
async function handleEvent(ev: any, pageId: string | null) {
  ...
  const systemPrompt = `اسمك هو "${botName}". إذا سألك أحد عن اسمك أو من أنت، عرّف نفسك بهذا الاسم دائماً. لا تذكر أنك Mistral أو أي نموذج آخر.
...
async function transcribeAudio(url: string): Promise<string | null> {
  ...
  try {
    ...
    else if (head.startsWith("ID3") || (bytes[0] === 0xff && (bytes[1] & 0xe0) === 0xe0)) { ext = "mp3"; sendType = "audio/mpeg"; }
    else if (head.includes("webm") || head.startsWith("\x1aE\xdf\xa3")) { ext = "webm"; sendType = "audio/webm"; }
    ...
    const attempt = async (model: string) => {
      ...
      if (!res.ok) {
        const t = await res.text().catch(() => "");
        console.error("[messenger] STT failed", model, res.status, t.slice(0, 400));
        return null;
      }
    ...
    let out = await attempt("openai/gpt-4o-transcribe");
    if (!out) out = await attempt("openai/gpt-4o-mini-transcribe");
    return out;
  } catch (err) {
  ...
  for (let step = 0; step < 6; step++) {
    ...
    try {
      ...
      if (tc.function?.name === "list_my_photos") {
        let parsed: any = null;
        try { parsed = JSON.parse(result); } catch { parsed = null; }
        if (parsed?.count > 0) return `تفضّل، هذه صورك المحفوظة 📷 (${parsed.count})\nلو أردت حذفها فقط قل «احذف صوري».`;
